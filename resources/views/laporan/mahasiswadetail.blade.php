...
<div style="padding-top:0px;">
    <a href="/mahasiswahistory/{{$stdid}}" type="button">
...
<div>
  <table class="table">
    ...
    <tbody>
      @foreach($mahasiswadetail as $mhsdet)
        <tr>
          <td>{{$mhsdet->Deskripsi}}</td>
          <td>{{$mhsdet->Mulai_Belajar}}</td>
          <td>{{$mhsdet->Mulai_Tes}}</td>
          <td>{{$mhsdet->Selesai_Tes}}</td>
          <td>{{$mhsdet->Score_Tes}}</td>
        </tr>
      @endforeach
    </tbody>           
  </table>
